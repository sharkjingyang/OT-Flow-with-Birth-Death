...
def plot_1d(net, x, nt_val, sPath, sTitle=""):
    ...
    gradPhi_full=torch.zeros(nSamples,nt_val+1)
    for i in range(nt_val+1):
        z=z_full[:,i].reshape(nSamples,1)
        z=pad(z[:, :d], (0, 1, 0, 0), value=i/nt_val)
        gradPhi, trH = net.trHess(z)
        # print(gradPhi.size())
        # print(gradPhi_full.size())
        gradPhi_full[:,i]=gradPhi[:,0]
        # print(i)


    sns.set_style('darkgrid')
    sns.displot(z_full[:,nt_val-1].cpu().numpy(),kind="kde")
    # plt.show()
    plt.savefig("image/hist/"+sTitle+".jpg")
    plt.close()
    ...
    for i in range(5):
        for j in range(5):
            k=i*5+j+1
            ax[i,j].scatter(z_full[:,k-1].cpu().numpy(),1/torch.exp(l_det[:,k-1]).cpu().numpy(),s=4)
            # ax[i,j].set_ylim(0,1)
            ax[i,j].set_ylabel("log(det)")
            ax[i,j].set_xlabel("z ")
    # plt.show()
    plt.savefig("image/logdet/"+sTitle+".jpg")
    plt.close()
    ...
    for i in range(5):
        for j in range(5):
            k=i*5+j+1
            ax[i,j].scatter(z_full[:,k-1].cpu().numpy(),gradPhi_full[:,k-1].cpu().numpy(),s=4)
            ax[i,j].set_ylabel("gradPhi(z)")
            ax[i,j].set_xlabel("z ")
    # plt.show()
    plt.savefig("image/gradPhi/"+sTitle+".jpg")
    plt.close()
    ...
    for i in range(5):
        for j in range(5):
            k=i*5+j+1
            x=net(pad(z_full[:,k-1].reshape(-1,d), (0, 1, 0, 0), value=1/nt_val*(k-1))).reshape(nSamples,-1)      
            ax[i,j].scatter(z_full[:,k-1].cpu().numpy(),x.cpu().numpy(),s=4)   
            ax[i,j].set_ylabel("Phi(z)")
            ax[i,j].set_xlabel("z ")
    # plt.show()
    plt.savefig("image/Phi/"+sTitle+".jpg")
    plt.close()
