...
# def deriv_one(x): # activation function aka the antiderivative of tanh
#     return torch.tanh(x)
def deriv_one(x): # activation function aka the antiderivative of tanh
    flag=(x>0).float()
    x=x*flag
...
class Phi(nn.Module):
    ...
    def trHess(self,x, justGrad=False ):
        ...
        tanhopen = deriv_one(opening) # act'( K_0 * S + b_0 )
        ...
        for i in range(N.nTh-1,0,-1): # work backwards, placing z_i in appropriate spot
            ...
            z[i] = term + N.h * torch.mm( N.layers[i].weight.t() , deriv_one( N.layers[i].forward(u[i-1]) ).t() * term)
        ...
        for i in range(1,N.nTh):
            ...
            Jac = Jac + N.h * deriv_one(temp).reshape(m, -1, nex) * KJ # update Jacobian
